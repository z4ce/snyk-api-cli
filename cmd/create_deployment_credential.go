...
func runCreateDeploymentCredential(cmd *cobra.Command, args []string) error {
	...
	if createDeploymentCredentialVerbose {
		fmt.Fprintf(os.Stderr, "* Requesting POST %s\n", fullURL)
	}

	// Create the HTTP request
	var body io.Reader
	if createDeploymentCredentialData != "" {
		body = strings.NewReader(createDeploymentCredentialData)
	}
	...
	} else if createDeploymentCredentialVerbose {
		fmt.Fprintf(os.Stderr, "* No automatic authorization available\n")
	}

	// Set user agent
	req.Header.Set("User-Agent", createDeploymentCredentialUserAgent)
	...
	return handleCreateDeploymentCredentialResponse(resp, createDeploymentCredentialIncludeResp, createDeploymentCredentialVerbose, createDeploymentCredentialSilent)
}
...
func handleCreateDeploymentCredentialResponse(resp *http.Response, includeResp, verbose, silent bool) error {
	if verbose {
